...
public class ItemSpecialController {
    ...
    @GetMapping
    public ResponseEntity<Map<String, Object>> getSpecialItems(@RequestParam Long userId) { //todo userId 수정
        return ResponseEntity.ok(itemSpecialService.getSpecialItems(userId));
    }

    @PostMapping("/order")
    public ResponseEntity<Map<String, Object>> purchaseSpecialItem(@RequestParam Long userId) { //todo userId 수정
